...
bool Server::ProcessPacket(int ID, PacketType _packettype)
{
	...
	case PacketType::ChatMessage: //Packet Type: chat message
	{
		...
		message = "\n" + connections[ID]->connectionName + " : " + message;
		...
		std::cout << "Processed chat message packet from user ID: " << ID << " message is " << message << std::endl;
		break;
	}
	...
	{	
		...
		if (canSet) {
			...
			connections[ID]->pm.Append(packettype);
			std::cout << "User ID" << ID << " cannot set username \n ";
